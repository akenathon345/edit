...
async function loadClientGuidelines(supabase, clientSlug) {
  ...
  try {
    ...
    // Strategy 4: input is a suffix of a Supabase slug (e.g. "valerie-duez-ruff" in "me-valerie-duez-ruff")
    if (!data) {
      const r4 = await supabase
        ...
        .ilike('client_slug', `%-${clientSlug}`)
        .limit(2);
      if (r4.data && r4.data.length === 1) {
        data = r4.data[0];
        console.log(`[guidelines] Fuzzy match (suffix): "${clientSlug}" → "${data.client_slug}"`);
      }
    }
