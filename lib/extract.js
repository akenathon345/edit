...
async function extractBundle(videoPath) {
  ...
  // Pre-check: verify video file exists and has content
  if (!fs.existsSync(videoPath)) {
    throw new Error(`Video file not found: ${videoPath}`);
  }
  const fileSize = fs.statSync(videoPath).size;
  console.log(`[extract] Video file: ${videoPath} (${(fileSize / 1024 / 1024).toFixed(1)} MB)`);
  if (fileSize < 1024) {
    throw new Error(`Video file too small (${fileSize} bytes) — download likely failed: ${videoPath}`);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      clearTimeout(timer);
      if (code !== 0) {
        const fullOutput = (stderr || stdout || '(no output)').trim();
        console.error(`[extract] Failed (code ${code}) stderr:`, stderr);
        console.error(`[extract] Failed (code ${code}) stdout:`, stdout);
        return reject(new Error(`extract.py failed (code ${code}): ${fullOutput.substring(0, 500)}`));
      }
