...
async function runPipeline(anthropic, supabase, { runId, videoPath, clientSlug, version, take, videoName, verbose }) {
  ...
  const costEstimate = (
    ...
    ((metrics.agents.a45_subtitle_qc?.tokensIn || 0) * 3 + (metrics.agents.a45_subtitle_qc?.tokensOut || 0) * 15) +
  ...
  return {
    ...
    if (bundlePath && !isBundle(videoPath)) {
      ...
      // GARDE-FOU : si le bundle est écrit à plat, dirname(bundlePath) = la
      // RACINE tmp — la supprimer emporterait uploads/ et les runs concurrents.
      const path = require('path');
      const parentDir = path.dirname(bundlePath);
      const target = path.resolve(parentDir) === path.resolve(BUNDLE_TMP_DIR) ? bundlePath : parentDir;
      cleanupPath(target);
    }
  }
}
