...
const { a1FrameDescriber, a2Cartographer, a3HookAuditor, a35HookVisionPass, a4BrollAuditor, a5QC, a6Formatter } = require('./agents');
...
async function runPipeline(anthropic, supabase, { runId, videoPath, clientSlug, version, take, videoName, verbose }) {
  ...
  // ── A3 + A3.5 + A4: Hook Auditor + Hook Vision + Broll Auditor (PARALLEL) ─
  console.log('\n── A3+A3.5+A4: Hook + Vision + Broll (parallel) ──');
  ...
  const [a3Settled, a4Settled, a35Settled] = await Promise.allSettled([
