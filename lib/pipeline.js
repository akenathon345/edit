...
async function runPipeline(anthropic, supabase, { runId, videoPath, clientSlug, version, take, videoName, verbose }) {
  ...
  // ── A3 + A4: Hook Auditor + Broll Auditor (PARALLEL) ────────────────────
  console.log('\n── A3+A4: Hook + Broll Auditors (parallel) ──────');
  const [a3Result, a4Result] = await Promise.all([
  ...
  metrics.agents.a3_hook_auditor = { tokensIn: a3Result.tokensIn, tokensOut: a3Result.tokensOut, duration_ms: a3Result.elapsed };
  metrics.agents.a4_broll_auditor = { tokensIn: a4Result.tokensIn, tokensOut: a4Result.tokensOut, duration_ms: a4Result.elapsed };
  console.log(`[A3] ${hookDirectives.length} hook directives`);
  console.log(`[A4] ${brollDirectives.length} broll directives`);
