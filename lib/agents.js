...
RÈGLE ENTITÉS NOMMÉES (illustration obligatoire) :
- Toute entité nommée à l'oral (personne publique, pays, institution, événement, lieu célèbre) doit être VISIBLE à l'écran dans les 2 secondes qui suivent sa mention.
- Parcours la transcription, repère chaque entité, vérifie qu'un plan la montre au bon moment. Sinon → AJOUTER_PLAN.
- Pour les personnes publiques et institutions réelles : propose des IMAGES RÉELLES (archives presse, plans publics, meetings, bains de foule) — PAS du stock générique anonyme. Ex : l'oral parle du président français → plan réel de Macron en bain de foule ; l'oral parle du Secret Service → plans publics réels d'agents du Secret Service.
...
- Ne flagge PAS les noms propres et marques, sauf faute interne évidente.
- Les sous-titres arrivent par fragments (karaoké) : reconstruis mentalement les phrases complètes avant de juger.
- En cas de doute → ne rien signaler. UNIQUEMENT les erreurs sûres.
  ...
  "subtitle_issues": [
    {
      ...
      "reason": "accord|orthographe|transcription|doublon"
    }
  ]
}
...
Détecte les erreurs dans les sous-titres incrustés. UNIQUEMENT les erreurs sûres.`;
  ...
  return await execAgent(anthropic, supabase, {
    ...
    content,
    maxTokens: 2000,
    model: process.env.A45_MODEL || MODELS.FORMAT,
    temperature: 0.1,
