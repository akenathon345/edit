...
- Pour les plans B-roll : note dans identifiable_elements les indices de NATIONALITÉ, d'ÉPOQUE et d'IDENTITÉ visibles (ex : "motard police française", "limousine présidentielle US", "drapeau américain", "immeuble haussmannien parisien", "agents Secret Service en tenue tactique"). Ces indices servent à vérifier la cohérence plan/propos.
...
// ── A3.5: HOOK VISION PASS ────────────────────────────────────────────────
// Passe vision directe sur les frames brutes de la zone hook (0-10s).
// Seul auditeur qui voit les pixels — détecte ce que les descriptions A1 ratent
// (absence de TS, confusion sous-titres/TS, B-roll plein écran).

const A35_SYSTEM = `Tu es un Virality Expert Édit (VE Edit) senior spécialisé dans les HOOKS visuels. Contrairement aux autres agents, tu REGARDES directement les frames de la zone hook (1 frame toutes les 0.5s). Tu es le seul auditeur qui voit les pixels : ta mission est de détecter ce que les descriptions texte ratent.

DISTINCTION CRITIQUE — TS vs SOUS-TITRES :
- Les SOUS-TITRES : retranscription de l'oral, mot à mot ou par petits groupes, qui CHANGE à chaque frame. Ce ne sont PAS des TS, même s'ils sont gros et centrés.
- Le TS (Titre Simple) : texte ÉDITORIAL en blocs, CAPS sur les mots importants, zone centrale haute, STABLE sur plusieurs secondes, qui ajoute une couche par-dessus l'oral.
- Une vidéo avec uniquement des sous-titres karaoké n'a PAS de TS → AJOUTER_TS obligatoire.

CE QUE TU VÉRIFIES (par ordre d'importance) :
1. TS présent dès 00:00 ? Un hook sans TS est un hook vide → AJOUTER_TS avec le texte exact (blocs + CAPS, curiosité pas description, ne répète JAMAIS l'oral, conforme au registre du client)
2. Speaker visible pendant les 3 premières secondes de chaque hook ?
3. B-roll PLEIN ÉCRAN qui masque le speaker dans le hook → PASSER EN ROTOSCOPIE (speaker détouré devant, plan derrière) ou VIRER si le plan n'apporte rien
4. Le FORMAT est-il compréhensible en 3 secondes (tier list, débunk, top 3, versus, storytelling) ? Le TS porte souvent le format.
5. Durée des plans dans le hook : statique 1s max, mouvement 1.5s max → RACCOURCIR
6. Zones rouges des guidelines client : logos interdits, visuels interdits, codes à éviter

RÈGLE TS ABSOLUE :
- UNE SEULE directive TS (AJOUTER_TS ou CHANGER_TS) par vidéo, au timecode le plus proche de 00:00
- Le texte proposé respecte le registre de langue et les mots interdits des guidelines client

Tu ne fais PAS d'observations. Tu fais des CHOIX exécutables par un monteur stagiaire. UNIQUEMENT les corrections nécessaires — si un élément est bon, tu n'en parles pas.

LES NIVEAUX DE DIRECTIVE :
VIRER / CHANGER / ROTOSCOPIE / RACCOURCIR / CHANGER_TS / AJOUTER_TS
  ...
  "hook_directives": [
    {
      ...
      "action": "AJOUTER_TS",
      "current_description": "ce qui est actuellement visible à l'écran",
      "instruction": "l'instruction concrète pour le monteur",
      "ts_text": "texte du TS si applicable (avec retours à la ligne pour les blocs)"
    }
  ]
}`;

async function a35HookVisionPass(anthropic, supabase, runId, { hookFrames, transcript, guidelines, videoMap }) {
  const MAX_FRAMES = 20; // 10s à 0.5s d'intervalle — contrôle du coût vision (~1.8k tokens/frame en prod)
  const framesToSend = hookFrames.slice(0, MAX_FRAMES);

  const content = [];
  content.push({
    type: 'text',
    text: `GUIDELINES (base + client) — registre TS, mots interdits, zones rouges :
${guidelines || '(guidelines de base uniquement)'}

TRANSCRIPTION ORALE DE LA ZONE HOOK (le TS ne doit JAMAIS la répéter) :
${transcript || '(non disponible)'}

HOOKS DÉTECTÉS PAR LA CARTOGRAPHIE :
${JSON.stringify(videoMap?.multi_hooks || [], null, 1)}

Voici les frames de la zone hook, 1 toutes les 0.5s :`,
  });
  ...
  for (const frame of framesToSend) {
    ...
    content.push({
      type: 'image',
      source: { type: 'base64', media_type: isPng ? 'image/png' : 'image/jpeg', data: frame.base64 },
    });
    content.push({
      type: 'text',
      text: `Frame ${frame.index} — ${frame.timecode}`,
    });
  }

  content.push({
    type: 'text',
    text: 'Produis les directives de montage pour le hook. UNIQUEMENT les corrections nécessaires.',
  });

  return await execAgent(anthropic, supabase, {
    runId,
    name: 'a35_hook_vision',
    systemPrompt: A35_SYSTEM,
    content,
    maxTokens: 3000,
    model: process.env.A35_MODEL || MODELS.AUDIT,
    temperature: 0.3,
    expectedShape: '{ "hook_directives": [{ "timecode": "00:00-00:06", "action": "AJOUTER_TS", "current_description": "...", "instruction": "...", "ts_text": "..." }] }',
  });
}
...
RÈGLE ENTITÉS NOMMÉES (illustration obligatoire) — MÉTHODE EN 2 ÉTAPES, À FAIRE EN PREMIER :
ÉTAPE 1 — Avant de juger les plans existants, fais l'INVENTAIRE des entités de la transcription : personnes publiques nommées OU désignées par leur fonction ("le président français" = Emmanuel Macron, "le président américain" = le président US en exercice), pays, institutions (Secret Service, GIGN, Élysée...), événements, lieux célèbres.
ÉTAPE 2 — Pour CHAQUE entité de l'inventaire : un plan la montre-t-elle dans les 2 secondes qui suivent sa mention ? Sinon → AJOUTER_PLAN.
- ATTENTION : montrer le cortège, le bâtiment ou l'institution n'est PAS montrer une personne. Si l'oral désigne une PERSONNE (même par sa fonction), il faut SON VISAGE à l'écran : image réelle publique (meeting, bain de foule, conférence de presse, interview).
- Pour les personnes publiques et institutions réelles : propose des IMAGES RÉELLES (archives presse, plans publics) — PAS du stock générique anonyme. Ex : "le président français" → plan réel de Macron en bain de foule ; "le Secret Service" → plans publics réels d'agents du Secret Service.
- Si le plan à ajouter peut être sensible (violence, panique, victimes réelles), le préciser : "attention plan sensible — choisir des images sans victimes identifiables".
- Exception : ne pas proposer une entité que les guidelines client interdisent explicitement à l'image.

RÈGLE COHÉRENCE FACTUELLE (le plan correspond au propos) :
- Pour CHAQUE plan de coupe, vérifie que la NATIONALITÉ / l'ÉPOQUE / l'IDENTITÉ de ce qui est montré correspond au référent de l'oral au même timecode.
- Utilise les identifiable_elements des frames : un convoi de police FRANÇAISE à l'écran pendant que l'oral parle du dispositif AMÉRICAIN = CHANGER avec le plan du bon pays.
- Un décalage de pays, d'époque, d'institution ou de contexte socio-pro = CHANGER, avec la description du plan correct.

LES 6 NIVEAUX DE DIRECTIVE :
1. VIRER → supprimer le plan, revenir sur le speaker
2. CHANGER → QUI + OÙ + QUOI + COMBIEN (durée). Description = mots-clés stock Pexels/Artgrid, OU référence réelle précise pour les entités publiques
3. PASSER EN ROTOSCOPIE → speaker devant, plan derrière
4. RACCOURCIR → durée cible en secondes
5. CHANGER TS → (rarement utilisé par A4)
6. AJOUTER_PLAN → insérer un plan là où il n'y en a pas : timecode précis de la mention orale + description exacte du plan à ajouter (QUI + OÙ + QUOI + durée). Images réelles pour les entités publiques.
  ...
  "broll_directives": [
    {
      ...
      "action": "VIRER|CHANGER|ROTOSCOPIE|RACCOURCIR|AJOUTER_PLAN",
      "current_description": "ce qui est actuellement visible (ou 'speaker seul, aucune illustration' pour AJOUTER_PLAN)",
      "instruction": "l'instruction concrète pour le monteur"
    }
  ]
}`;

async function a4BrollAuditor(anthropic, supabase, runId, { videoMap, frameDescriptions, guidelines, skillSection, transcript }) {
...
TRANSCRIPTION ORALE (pour la règle entités nommées et la cohérence factuelle) :
${(transcript || '(non disponible)').substring(0, 4000)}

TOUTES LES DESCRIPTIONS DES FRAMES :
${JSON.stringify(frameDescriptions, null, 1)}

Produis les directives de montage pour TOUS les plans de coupe. UNIQUEMENT les corrections nécessaires. N'oublie pas la règle ENTITÉS NOMMÉES : chaque personne publique / pays / institution mentionnée à l'oral doit être visualisée — sinon AJOUTER_PLAN.`;
...
// ── A4.5: SUBTITLE QA ─────────────────────────────────────────────────────
// Relit les sous-titres incrustés (extraits visuellement par A1) contre la
// transcription audio + les règles du français. Texte seul, quasi gratuit.

const A45_SYSTEM = `Tu es un correcteur de sous-titres incrustés pour des vidéos courtes en français.

Tu reçois :
1. La séquence des sous-titres INCRUSTÉS dans la vidéo (lus à l'écran frame par frame, dans l'ordre chronologique)
2. La transcription audio de référence (Whisper) — ATTENTION : elle peut elle-même contenir des erreurs

TON JOB — détecter dans les sous-titres INCRUSTÉS :
1. FAUTES CERTAINES de français : orthographe, grammaire, accords (genre/nombre), conjugaison. Ex : "une effectif" → "un effectif". Cibles prioritaires — signale-les même si la transcription est douteuse.
2. ERREURS DE TRANSCRIPTION évidentes : le sous-titre ne correspond pas à l'oral ET n'a aucun sens dans le contexte. Ex : l'oral dit "cas non conformes", le sous-titre affiche "canaux conformes" (absurde dans le contexte).
3. MOTS MANQUANTS qui changent le sens, ou doublons de mots.

PRUDENCE :
- Whisper se trompe aussi : si le sous-titre incrusté est plausible et correct en français, NE PAS le flagger juste parce qu'il diffère de Whisper.
- Ne flagge PAS le style : découpage karaoké, absence de ponctuation, majuscules, abréviations volontaires — ce sont des choix de montage.
- NOMS PROPRES ET TERMES ÉTRANGERS = JAMAIS de correction : "Secret Service", "secret services", noms de marques, anglicismes, noms de personnes/lieux. Les règles d'accord du français NE S'APPLIQUENT PAS aux noms propres étrangers.
- Les expressions orales familières ("travailler en lourd", "ça passe crème") ne sont PAS des fautes — c'est ce que dit le speaker.
- Les sous-titres arrivent par fragments (karaoké) : reconstruis mentalement les phrases complètes avant de juger.
- Si la transcription de référence est absente ou "(non disponible)" : signale UNIQUEMENT les fautes de français pures (accord, orthographe, conjugaison), jamais de raison "transcription".
- En cas de doute → ne rien signaler. UNIQUEMENT les erreurs 100% sûres.

FORMAT STRICT :
- "timecode" : recopie EXACTEMENT la valeur entre crochets du fragment concerné, sans la reformater ni la convertir.
- "correction" : le texte corrigé, SEUL. INTERDIT : alternatives ("ou possiblement..."), parenthèses, commentaires, points d'interrogation.
- "reason" : un seul mot parmi accord | orthographe | conjugaison | transcription | doublon.
...
SORTIE JSON :
{
  "subtitle_issues": [
    {
      "timecode": "21.0s",
      "current_text": "le texte fautif tel qu'affiché",
      "correction": "le texte corrigé",
      "reason": "accord"
    }
  ]
}
Si aucune erreur : { "subtitle_issues": [] }`;

async function a45SubtitleQC(anthropic, supabase, runId, { frameDescriptions, transcript }) {
  // Reconstruire la timeline des sous-titres incrustés depuis les descriptions A1
  const subtitleTimeline = (frameDescriptions || [])
    .filter(f => f.subtitle_text)
    .map(f => `[${f.timecode}] ${f.subtitle_text}`)
    .join('\n');

  if (!subtitleTimeline) {
    return { parsed: { subtitle_issues: [] }, raw: '', qcStatus: 'skipped', tokensIn: 0, tokensOut: 0, elapsed: 0 };
  }

  const content = `SOUS-TITRES INCRUSTÉS (lus à l'écran, frame par frame) :
${subtitleTimeline}

TRANSCRIPTION AUDIO DE RÉFÉRENCE (Whisper — peut contenir ses propres erreurs) :
${(transcript || '(non disponible)').substring(0, 4000)}

Détecte les erreurs dans les sous-titres incrustés. UNIQUEMENT les erreurs sûres.`;

  return await execAgent(anthropic, supabase, {
    runId,
    name: 'a45_subtitle_qc',
    systemPrompt: A45_SYSTEM,
    content,
    maxTokens: 2000,
    model: process.env.A45_MODEL || MODELS.ANALYSIS,
    temperature: 0.1,
    expectedShape: '{ "subtitle_issues": [{ "timecode": "00:21", "current_text": "...", "correction": "...", "reason": "accord" }] }',
  });
}
...
5. SÉLECTIVITÉ : garde UNIQUEMENT les directives à fort impact. Le monteur ne veut pas 15 retours dont 10 mineurs — il veut les 5-8 corrections qui changent vraiment la vidéo. En cas de doute, SUPPRIMER la directive.
   EXCEPTION : les AJOUTER_PLAN qui visualisent une entité nommée à l'oral (personne publique, pays, institution) et les CHANGER pour incohérence factuelle (mauvais pays/époque à l'écran) sont par définition à FORT impact — ne les supprime pas pour la sélectivité, seulement s'ils sont doublons ou hors sujet.
  ...
  "directives": [
    {
      ...
      "action": "VIRER|CHANGER|ROTOSCOPIE|RACCOURCIR|CHANGER_TS|AJOUTER_TS|AJOUTER_PLAN",
      "current_description": "...",
      "instruction": "...",
...
CORRECTIONS SOUS-TITRES (si fournies dans l'entrée) :
Après la dernière directive de montage, ajouter UNE directive par correction, au format :

### TIMECODE | SOUS-TITRE

« texte fautif » → « texte corrigé »

SI LE TABLEAU DE DIRECTIVES EST VIDE ET AUCUNE CORRECTION SOUS-TITRE :
...
async function a6Formatter(anthropic, supabase, runId, { qcOutput, videoMap, metadata, subtitleIssues }) {
  const content = `DIRECTIVES VALIDÉES :
${JSON.stringify(qcOutput.directives, null, 1)}

CORRECTIONS SOUS-TITRES :
${JSON.stringify(subtitleIssues || [], null, 1)}
...
module.exports = { a1FrameDescriber, a2Cartographer, a3HookAuditor, a35HookVisionPass, a4BrollAuditor, a45SubtitleQC, a5QC, a6Formatter };
