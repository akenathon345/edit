...
async function a1FrameDescriber(anthropic, supabase, runId, frames) {
  ...
  for (const result of batchResults) {
    ...
    totalElapsed = Math.max(totalElapsed, result.elapsed); // parallel = max
  }
...
2. DÉDUPLICATION : si hook et B-roll ont produit des directives sur le même timecode, fusionner

3. ORDRE CHRONOLOGIQUE : trier toutes les directives par timecode croissant

4. TOLÉRANCE MARQUE : la transcription audio peut mal orthographier la marque du client (ex : "EZAK" → "HEYZACK"). Si une directive demande de virer/flouter ce qui est en fait la marque du client, SUPPRIMER cette directive.

5. SCORE /10 : évaluer la qualité du montage original
